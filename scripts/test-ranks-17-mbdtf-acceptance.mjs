// Focused acceptance test for rank 17 — Kanye West, My Beautiful Dark Twisted Fantasy.
// Verifies the completed immutable unpublished edition 1: exact catalog identity
// against the committed 14-track baseline (13 disc-1 + 1 disc-2 bonus), documented
// evidence with supported independent review for every documented track.
...
assert.ok(entry, 'rank-17 authoring entry must exist');
assert.equal(entry.editionNumber, 1, 'rank-17 must be edition 1');
assert.equal(entry.published, false, 'rank-17 edition must remain unpublished');
assert.match(entry.contentHash ?? '', /^[a-f0-9]{16}$/, 'rank-17 must carry a semantic content hash');
...
assert.deepEqual(entryKeys, catalogKeys, 'rank-17 track identities must exactly match catalog');
assert.equal(entry.trackEntries.length, 14, 'rank-17 must preserve all fourteen catalog tracks');
...
assert.equal(documented, 14, 'rank-17 must document all 14 catalog tracks');
assert.equal(insufficient, 0, 'rank-17 must carry zero insufficient-evidence tracks');

console.log(`rank-17 My Beautiful Dark Twisted Fantasy acceptance test passed: 14 tracks (14 documented, 0 insufficient-evidence), exact catalog identity, unpublished edition 1.`);
