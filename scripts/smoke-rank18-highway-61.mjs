// Real Chrome production-preview smoke for rank 18 — Bob Dylan, Highway 61 Revisited.
// Drives a headless Chrome over CDP, opens the rank-18 album, and captures
...
const chrome = spawn(CHROME, [
  ...
  `--remote-debugging-port=${PORT}`, '--user-data-dir=/tmp/albumvault-smoke-profile-r18',
  'about:blank',
], { stdio: 'ignore' });
...
try {
  ...
  const clickResult = await cdp.send('Runtime.evaluate', {
    expression: `(() => {
      ...
      const target = btns.find(b => (b.textContent || '').includes('Highway 61 Revisited') && (b.textContent || '').includes('Bob Dylan'));
  ...
  const state = await cdp.send('Runtime.evaluate', {
    expression: `(() => {
      ...
      const hasAlbum = body.includes('Highway 61 Revisited') && body.includes('Bob Dylan');
      const hasTrack = body.includes('Like a Rolling Stone') || body.includes('Tombstone Blues') || body.includes('Desolation Row');
      const hasEvidence = body.includes('Michael Bloomfield') || body.includes('Al Kooper') || body.includes('Bill Janovitz') || body.includes('Andy Gill');
