// Focused acceptance test for rank 26 — Patti Smith, Horses.
...
assert.ok(entry, 'rank-26 authoring entry must exist');
assert.equal(entry.editionNumber, 1, 'rank-26 must be edition 1');
assert.equal(entry.published, false, 'rank-26 edition must remain unpublished');
assert.match(entry.contentHash ?? '', /^[a-f0-9]{16}$/, 'rank-26 must carry a semantic content hash');
...
assert.deepEqual(entryKeys, catalogKeys, 'rank-26 track identities must exactly match catalog');
assert.equal(entry.trackEntries.length, 8, 'rank-26 must preserve all eight catalog tracks');
...
assert.equal(documented, 8, 'rank-26 must document all 8 catalog tracks');
assert.equal(insufficient, 0, 'rank-26 must carry zero insufficient-evidence tracks');

console.log(`rank-26 Horses acceptance test passed: 8 tracks (8 documented, 0 insufficient-evidence), exact catalog identity, unpublished edition 1.`);
