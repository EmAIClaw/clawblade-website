...
require_clean_except "$HOST_REPO" '^(dist-gym/|scripts/build-gym-tracker\.sh|scripts/deploy-gym-via-github\.sh|package\.json)$'
commit_if_needed "$HOST_REPO" "$HOST_MESSAGE" dist-gym scripts/build-gym-tracker.sh scripts/deploy-gym-via-github.sh package.json
