// Real Chrome production-preview smoke for rank 21 — Bruce Springsteen, Born to Run.
// Drives a headless Chrome over CDP, opens the rank-21 album, and captures
...
const chrome = spawn(CHROME, [
  ...
  `--remote-debugging-port=${PORT}`, '--user-data-dir=/tmp/albumvault-smoke-profile-r21',
  'about:blank',
], { stdio: 'ignore' });
...
try {
  ...
  const clickResult = await cdp.send('Runtime.evaluate', {
    expression: `(() => {
      ...
      const target = btns.find(b => (b.textContent || '').includes('Born to Run') && (b.textContent || '').includes('Springsteen'));
  ...
  const state = await cdp.send('Runtime.evaluate', {
    expression: `(() => {
      ...
      const hasAlbum = body.includes('Born to Run') && body.includes('Springsteen');
      const hasTrack = body.includes('Thunder Road') || body.includes('Jungleland') || body.includes('Backstreets') || body.includes('Tenth Avenue Freeze-Out');
      const hasEvidence = body.includes('Long Branch') || body.includes('E Street Band') || body.includes('Suki Lahav') || body.includes('Randy Brecker') || body.includes('Roy Bittan');
