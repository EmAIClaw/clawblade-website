// Real Chrome production-preview smoke for rank 11 — The Beatles, Revolver.
// Drives a headless Chrome over CDP, opens the rank-11 album, and captures
...
const chrome = spawn(CHROME, [
  ...
  `--remote-debugging-port=${PORT}`, '--user-data-dir=/tmp/albumvault-smoke-profile-r11',
  'about:blank',
], { stdio: 'ignore' });
...
try {
  ...
  const clickResult = await cdp.send('Runtime.evaluate', {
    expression: `(() => {
      ...
      const target = btns.find(b => (b.textContent || '').includes('Revolver'));
  ...
  const state = await cdp.send('Runtime.evaluate', {
    expression: `(() => {
      ...
      const hasAlbum = body.includes('Revolver');
      const hasTrack = body.includes('Tomorrow Never Knows') || body.includes('Eleanor Rigby');
      const hasEvidence = body.includes('Harold Wilson') || body.includes('string octet') || body.includes('Peter Fonda') || body.includes('Freymann');
      const hasSource = body.includes('Beatles Bible');
