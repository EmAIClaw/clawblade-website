// Focused acceptance test for rank 36 — Michael Jackson, Off the Wall.
// Verifies the completed immutable unpublished edition 1: exact catalog identity
// against the committed 10-track baseline, documented evidence with supported
...
assert.ok(entry, 'rank-36 authoring entry must exist');
assert.equal(entry.editionNumber, 1, 'rank-36 must be edition 1');
assert.equal(entry.published, false, 'rank-36 edition must remain unpublished');
assert.match(entry.contentHash ?? '', /^[a-f0-9]{16}$/, 'rank-36 must carry a semantic content hash');
...
assert.deepEqual(entryKeys, catalogKeys, 'rank-36 track identities must exactly match catalog');
assert.equal(entry.trackEntries.length, 10, 'rank-36 must preserve all ten catalog tracks');
...
assert.equal(documented, 10, 'rank-36 must document all 10 catalog tracks');
assert.equal(insufficient, 0, 'rank-36 must carry zero insufficient-evidence tracks');

console.log(`rank-36 Off the Wall acceptance test passed: 10 tracks (10 documented, 0 insufficient-evidence), exact catalog identity, unpublished edition 1.`);
