...
const selectedRanks = [36, 37, 38, 39, 40, 41, 42];
...
assert.deepEqual(selectedAlbums.map((album) => album.rank), selectedRanks, 'research scope must cover rank 36 plus its next six deficient ranks');
...
console.log(`ranks 36–42 track-research regression test passed: ${sourced.length} sourced guides across ${perAlbum.size} albums`);
