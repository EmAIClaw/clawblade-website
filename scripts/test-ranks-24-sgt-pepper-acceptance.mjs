// Focused acceptance test for rank 24 — The Beatles, Sgt. Pepper's Lonely Hearts Club Band.
// Verifies the completed immutable unpublished edition 1: exact catalog identity
// against the committed 13-track baseline, documented evidence with supported
...
assert.ok(entry, 'rank-24 authoring entry must exist');
assert.equal(entry.editionNumber, 1, 'rank-24 must be edition 1');
assert.equal(entry.published, false, 'rank-24 edition must remain unpublished');
assert.match(entry.contentHash ?? '', /^[a-f0-9]{16}$/, 'rank-24 must carry a semantic content hash');
...
assert.deepEqual(entryKeys, catalogKeys, 'rank-24 track identities must exactly match catalog');
assert.equal(entry.trackEntries.length, 13, 'rank-24 must preserve all thirteen catalog tracks');
...
assert.equal(documented, 13, 'rank-24 must document all 13 catalog tracks');
assert.equal(insufficient, 0, 'rank-24 must carry zero insufficient-evidence tracks');

console.log('rank-24 Sgt. Pepper acceptance test passed: 13 tracks (13 documented, 0 insufficient-evidence), exact catalog identity, unpublished edition 1.');
