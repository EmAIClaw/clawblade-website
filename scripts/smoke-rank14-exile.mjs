// Real Chrome production-preview smoke for rank 14 — The Rolling Stones, Exile on Main St.
// Drives a headless Chrome over CDP, opens the rank-14 album, and captures
...
const chrome = spawn(CHROME, [
  ...
  `--remote-debugging-port=${PORT}`, '--user-data-dir=/tmp/albumvault-smoke-profile-r14',
  'about:blank',
], { stdio: 'ignore' });
...
try {
  ...
  const clickResult = await cdp.send('Runtime.evaluate', {
    expression: `(() => {
      ...
      const target = btns.find(b => (b.textContent || '').includes('Exile on Main St') && (b.textContent || '').includes('Rolling Stones'));
  ...
  const state = await cdp.send('Runtime.evaluate', {
    expression: `(() => {
      ...
      const hasAlbum = body.includes('Exile on Main St') && body.includes('Rolling Stones');
      const hasTrack = body.includes('Rocks Off') || body.includes('Tumbling Dice') || body.includes('Soul Survivor');
      const hasEvidence = body.includes('Andy Johns') || body.includes('Gram Parsons') || body.includes('Mick Taylor') || body.includes('Nellcôte') || body.includes('Nellcote');
      const hasSource = body.includes('Rolling Stone') || body.includes('Track By Track');
