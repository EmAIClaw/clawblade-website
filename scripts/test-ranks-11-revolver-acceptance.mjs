// Focused acceptance test for rank 11 — The Beatles, Revolver.
...
assert.ok(entry, 'rank-11 authoring entry must exist');
assert.equal(entry.editionNumber, 1, 'rank-11 must be edition 1');
assert.equal(entry.published, false, 'rank-11 edition must remain unpublished');
assert.match(entry.contentHash ?? '', /^[a-f0-9]{16}$/, 'rank-11 must carry a semantic content hash');
...
assert.deepEqual(entryKeys, catalogKeys, 'rank-11 track identities must exactly match catalog');
assert.equal(entry.trackEntries.length, 14, 'rank-11 must preserve all fourteen catalog tracks');
...
console.log('rank-11 Revolver acceptance test passed: 14 documented tracks, 14 supported review decisions, exact catalog identity, unpublished edition 1.');
