// Focused acceptance test for rank 18 — Bob Dylan, Highway 61 Revisited.
// Verifies the completed immutable unpublished edition 1: exact catalog identity
// against the committed 9-track baseline, documented evidence with supported
...
assert.ok(entry, 'rank-18 authoring entry must exist');
assert.equal(entry.editionNumber, 1, 'rank-18 must be edition 1');
assert.equal(entry.published, false, 'rank-18 edition must remain unpublished');
assert.match(entry.contentHash ?? '', /^[a-f0-9]{16}$/, 'rank-18 must carry a semantic content hash');
...
assert.deepEqual(entryKeys, catalogKeys, 'rank-18 track identities must exactly match catalog');
assert.equal(entry.trackEntries.length, 9, 'rank-18 must preserve all nine catalog tracks');
...
assert.equal(documented, 9, 'rank-18 must document all 9 catalog tracks');
assert.equal(insufficient, 0, 'rank-18 must carry zero insufficient-evidence tracks');

console.log(`rank-18 Highway 61 Revisited acceptance test passed: 9 tracks (9 documented, 0 insufficient-evidence), exact catalog identity, unpublished edition 1.`);
