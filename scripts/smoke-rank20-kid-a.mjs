// Real Chrome production-preview smoke for rank 20 — Radiohead, Kid A.
// Drives a headless Chrome over CDP, opens the rank-20 album, and captures
...
const chrome = spawn(CHROME, [
  ...
  `--remote-debugging-port=${PORT}`, '--user-data-dir=/tmp/albumvault-smoke-profile-r20',
  'about:blank',
], { stdio: 'ignore' });
...
try {
  ...
  const exceptions = [];
  cdp.onEvent((msg) => {
    if (msg.method === 'Runtime.exceptionThrown') exceptions.push(msg.params.exceptionDetails?.text ?? 'unknown exception');
  ...
  const clickResult = await cdp.send('Runtime.evaluate', {
    expression: `(() => {
      ...
      const target = btns.find(b => (b.textContent || '').includes('Kid A') && (b.textContent || '').includes('Radiohead'));
  ...
  const state = await cdp.send('Runtime.evaluate', {
    expression: `(() => {
      ...
      const hasAlbum = body.includes('Kid A') && body.includes('Radiohead');
      const hasTrack = body.includes('Everything in Its Right Place') || body.includes('Idioteque') || body.includes('Motion Picture Soundtrack') || body.includes('The National Anthem');
      const hasEvidence = body.includes('depression') || body.includes('Dorchester Abbey') || body.includes('Lost At Sea') || body.includes('modular synthesiser') || body.includes('hidden track');
      const hasSource = body.includes('Wikipedia') || body.includes('Songfacts') || body.includes('Citizen Insane');
  ...
  const ok = state.result.value.hasAlbum && state.result.value.hasTrack &&
    state.result.value.hasEvidence && state.result.value.hasSource && exceptions.length === 0;
