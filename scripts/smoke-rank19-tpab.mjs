// Real Chrome production-preview smoke for rank 19 — Kendrick Lamar, To Pimp a Butterfly.
// Drives a headless Chrome over CDP, opens the rank-19 album, and captures
...
const chrome = spawn(CHROME, [
  ...
  `--remote-debugging-port=${PORT}`, '--user-data-dir=/tmp/albumvault-smoke-profile-r19',
  'about:blank',
], { stdio: 'ignore' });
...
try {
  ...
  const clickResult = await cdp.send('Runtime.evaluate', {
    expression: `(() => {
      ...
      const target = btns.find(b => (b.textContent || '').includes('To Pimp a Butterfly') && (b.textContent || '').includes('Kendrick Lamar'));
  ...
  const state = await cdp.send('Runtime.evaluate', {
    expression: `(() => {
      ...
      const hasAlbum = body.includes('To Pimp a Butterfly') && body.includes('Kendrick Lamar');
      const hasTrack = body.includes('Wesley') || body.includes('King Kunta') || body.includes('Mortal Man') || body.includes('Alright');
      const hasEvidence = body.includes('Wesley Snipes') || body.includes('Robert Glasper') || body.includes('Sufjan Stevens') || body.includes('Rapsody') || body.includes('Knxwledge');
      const hasSource = body.includes('Wikipedia') || body.includes('Stereogum');
