...
const selectedAlbums = (catalogData.albums ?? []).filter((album) => album.rank >= 58 && album.rank <= 64);
...
assert.equal(selectedAlbums.length, 7, 'research scope must remain catalog ranks 58–64');
...
console.log(`ranks 58–64 research regression test passed: ${sourced.length} source-backed guides across ${perAlbum.size} albums`);
