...
const selectedAlbums = (catalogData.albums ?? []).filter((album) => album.rank >= 65 && album.rank <= 71);
const entries = encyclopediaData.entries ?? {};
const sourced = selectedAlbums.flatMap((album) =>
  (entries[album.id]?.trackGuide ?? []).filter((guide) => guide.source?.url).map((guide) => ({ album, guide }))
);
const perAlbum = new Map();
for (const { album } of sourced) perAlbum.set(album.id, (perAlbum.get(album.id) ?? 0) + 1);

assert.equal(selectedAlbums.length, 7, 'research scope must remain catalog ranks 65–71');
...
assert.ok(selectedAlbums.every((album) => (perAlbum.get(album.id) ?? 0) >= 3), 'every selected album requires at least three source-backed notes');
...
console.log(`ranks 65–71 research regression test passed: ${sourced.length} source-backed guides across ${perAlbum.size} albums`);
