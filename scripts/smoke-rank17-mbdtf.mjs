// Real Chrome production-preview smoke for rank 17 — Kanye West, My Beautiful Dark Twisted Fantasy.
// Drives a headless Chrome over CDP, opens the rank-17 album, and captures
...
const chrome = spawn(CHROME, [
  ...
  `--remote-debugging-port=${PORT}`, '--user-data-dir=/tmp/albumvault-smoke-profile-r17',
  'about:blank',
], { stdio: 'ignore' });
...
try {
  ...
  const clickResult = await cdp.send('Runtime.evaluate', {
    expression: `(() => {
      ...
      const target = btns.find(b => (b.textContent || '').includes('My Beautiful Dark Twisted Fantasy') && (b.textContent || '').includes('Kanye West'));
  ...
  const state = await cdp.send('Runtime.evaluate', {
    expression: `(() => {
      ...
      const hasAlbum = body.includes('My Beautiful Dark Twisted Fantasy') && body.includes('Kanye West');
      const hasTrack = body.includes('Dark Fantasy') || body.includes('Runaway') || body.includes('Power') || body.includes('Monster');
      const hasEvidence = body.includes('Nicki Minaj') || body.includes('King Crimson') || body.includes('Bon Iver') || body.includes('Smokey Robinson');
      const hasSource = body.includes('Wikipedia');
