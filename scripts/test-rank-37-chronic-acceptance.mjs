// Focused acceptance test for rank 37 — Dr. Dre, The Chronic.
// Verifies immutable unpublished edition 1: exact catalog identity against the
// committed 16-track baseline, documented tracks backed by retained source
// artifacts and supported independent review, and no unresearched tracks.
...
assert.ok(album, 'rank-37 catalog album must exist');
assert.ok(entry, 'rank-37 authoring entry must exist');
assert.equal(entry.editionNumber, 1, 'rank-37 must be edition 1');
assert.equal(entry.published, false, 'rank-37 edition must remain unpublished');
assert.match(entry.contentHash ?? '', /^[a-f0-9]{16}$/, 'rank-37 must carry a semantic content hash');
...
assert.deepEqual(entryKeys, catalogKeys, 'rank-37 track identities must exactly match catalog');
assert.equal(entry.trackEntries.length, 16, 'rank-37 must preserve all sixteen catalog tracks');
...
for (const track of entry.trackEntries) {
  if (track.evidenceLevel === 'documented') {
    ...
    assert.ok(Array.isArray(d.searchedQueries) && d.searchedQueries.length >= 2, `${track.trackTitle}: searchedQueries coverage required`);
...
assert.equal(documented + insufficient, 16, 'rank-37 must leave zero unresearched tracks');
assert.ok(documented >= 3, 'rank-37 must retain documented claims where source support exists');

console.log(`rank-37 The Chronic acceptance test passed: 16 tracks (${documented} documented, ${insufficient} insufficient-evidence), exact catalog identity, unpublished edition 1.`);
