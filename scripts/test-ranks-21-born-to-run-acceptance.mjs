// Focused acceptance test for rank 21 — Bruce Springsteen, Born to Run.
// Verifies the completed immutable unpublished edition 1: exact catalog identity
// against the committed 8-track baseline, documented evidence with supported
...
assert.ok(entry, 'rank-21 authoring entry must exist');
assert.equal(entry.editionNumber, 1, 'rank-21 must be edition 1');
assert.equal(entry.published, false, 'rank-21 edition must remain unpublished');
assert.match(entry.contentHash ?? '', /^[a-f0-9]{16}$/, 'rank-21 must carry a semantic content hash');
...
assert.deepEqual(entryKeys, catalogKeys, 'rank-21 track identities must exactly match catalog');
assert.equal(entry.trackEntries.length, 8, 'rank-21 must preserve all eight catalog tracks');
...
assert.equal(documented, 8, 'rank-21 must document all 8 catalog tracks');
assert.equal(insufficient, 0, 'rank-21 must carry zero insufficient-evidence tracks');

console.log(`rank-21 Born to Run acceptance test passed: 8 tracks (8 documented, 0 insufficient-evidence), exact catalog identity, unpublished edition 1.`);
