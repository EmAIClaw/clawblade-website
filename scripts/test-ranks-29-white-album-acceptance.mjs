// Focused acceptance test for rank 29 — The Beatles, The Beatles (White Album).
// Verifies the completed immutable unpublished edition 1: exact catalog identity
// against the committed 30-track baseline, documented/insufficient-evidence
// dispositions for every track, and supported independent review for every
// documented claim.
...
assert.ok(entry, 'rank-29 authoring entry must exist');
assert.equal(entry.editionNumber, 1, 'rank-29 must be edition 1');
assert.equal(entry.published, false, 'rank-29 edition must remain unpublished');
assert.match(entry.contentHash ?? '', /^[a-f0-9]{16}$/, 'rank-29 must carry a semantic content hash');
...
assert.deepEqual(entryKeys, catalogKeys, 'rank-29 track identities must exactly match catalog');
assert.equal(entry.trackEntries.length, 30, 'rank-29 must preserve all thirty catalog tracks');
...
assert.equal(documented + insufficient, 30, 'rank-29 must resolve every catalog track');
assert.equal(documented, 30, 'rank-29 should document all 30 White Album tracks when source-backed research succeeds');
assert.equal(insufficient, 0, 'rank-29 must carry zero insufficient-evidence tracks when source-backed research succeeds');

console.log(`rank-29 The Beatles (White Album) acceptance test passed: 30 tracks (${documented} documented, ${insufficient} insufficient-evidence), exact catalog identity, unpublished edition 1.`);
