// Focused acceptance test for rank 20 — Radiohead, Kid A.
...
assert.ok(entry, 'rank-20 authoring entry must exist');
assert.equal(entry.editionNumber, 1, 'rank-20 must be edition 1');
assert.equal(entry.published, false, 'rank-20 edition must remain unpublished');
assert.match(entry.contentHash ?? '', /^[a-f0-9]{16}$/, 'rank-20 must carry a semantic content hash');
...
assert.deepEqual(entryKeys, catalogKeys, 'rank-20 track identities must exactly match catalog');
assert.equal(entry.trackEntries.length, 11, 'rank-20 must preserve all eleven catalog tracks');
...
assert.equal(documented, 11, 'rank-20 must document all 11 catalog tracks');
assert.equal(insufficient, 0, 'rank-20 must carry zero insufficient-evidence tracks');

console.log(`rank-20 Kid A acceptance test passed: 11 tracks (11 documented, 0 insufficient-evidence), exact catalog identity, unpublished edition 1.`);
