...
const selectedRanks = [76, 83, 88, 90, 91];
...
assert.deepEqual(selectedAlbums.map((album) => album.rank), selectedRanks, 'research scope must remain the next five catalog coverage gaps with adequate direct evidence');
assert.ok(sourced.length >= 15, `expected at least 15 substantive source-backed notes, found ${sourced.length}`);
...
console.log(`ranks 76–91 coverage-gap regression test passed: ${sourced.length} sourced guides across ${perAlbum.size} albums`);
