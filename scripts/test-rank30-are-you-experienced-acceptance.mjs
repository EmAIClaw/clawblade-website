// Focused acceptance test for rank 30 — The Jimi Hendrix Experience, Are You Experienced.
// Verifies the completed immutable unpublished edition 1: exact catalog identity
// against the committed 17-track deluxe baseline, documented evidence with supported
...
const ALBUM_ID = '030-the-jimi-hendrix-experience-are-you-experienced-f4ea672f';
...
assert.ok(entry, 'rank-30 authoring entry must exist');
assert.equal(entry.editionNumber, 1, 'rank-30 must be edition 1');
assert.equal(entry.published, false, 'rank-30 edition must remain unpublished');
assert.match(entry.contentHash ?? '', /^[a-f0-9]{16}$/, 'rank-30 must carry a semantic content hash');
...
assert.deepEqual(entryKeys, catalogKeys, 'rank-30 track identities must exactly match catalog');
assert.equal(entry.trackEntries.length, 17, 'rank-30 must preserve all seventeen catalog tracks');
...
assert.equal(documented, 17, 'rank-30 must document all 17 catalog tracks');
assert.equal(insufficient, 0, 'rank-30 must carry zero insufficient-evidence tracks');

console.log(`rank-30 Are You Experienced acceptance test passed: 17 tracks (17 documented, 0 insufficient-evidence), exact catalog identity, unpublished edition 1.`);
