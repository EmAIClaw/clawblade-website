// Real Chrome production-preview smoke for rank 12 — Michael Jackson, Thriller.
// Drives a headless Chrome over CDP, opens the rank-12 album, and captures
...
const chrome = spawn(CHROME, [
  ...
  `--remote-debugging-port=${PORT}`, '--user-data-dir=/tmp/albumvault-smoke-profile-r12',
  'about:blank',
], { stdio: 'ignore' });
...
try {
  ...
  const clickResult = await cdp.send('Runtime.evaluate', {
    expression: `(() => {
      ...
      const target = btns.find(b => (b.textContent || '').includes('Thriller') && (b.textContent || '').includes('Michael Jackson'));
  ...
  const state = await cdp.send('Runtime.evaluate', {
    expression: `(() => {
      ...
      const hasAlbum = body.includes('Thriller') && body.includes('Michael Jackson');
      const hasTrack = body.includes('Billie Jean') || body.includes('Human Nature');
      const hasEvidence = body.includes('Louis Johnson') || body.includes('Univox') || body.includes('wolf howls') || body.includes('Synclavier');
      const hasSource = body.includes('Thriller 40') || body.includes('Bruce Swedien');
