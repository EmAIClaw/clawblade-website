...
const selectedRanks = [18, 19, 20, 22, 23, 26, 31];
...
assert.deepEqual(selectedAlbums.map((album) => album.rank), selectedRanks, 'research scope must remain the lowest rank-18 coverage gap plus its next six deficient ranks');
...
console.log(`ranks 18–31 coverage-gap regression test passed: ${sourced.length} sourced guides across ${perAlbum.size} albums`);
