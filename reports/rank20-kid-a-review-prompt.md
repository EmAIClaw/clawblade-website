...
Review input JSON: reports/rank20-kid-a-review-input.json
...
The review input JSON has this exact structure (do NOT explore with jq; just read the file):
{
  ...
  "candidate": {
    "albumId": "...",
    "editionNumber": 1,
    "generationMetadata": { "generator": "codex-rank20-track-encyclopedia-author", ... },
    "trackEntries": [
      {
...
- For each candidate verified fact (11 total, one per track), verify mechanically and semantically:
...
Output only one JSON object to stdout, no markdown, no prose, using this exact shape:
{
  ...
  "reviewer": {
    ...
    "runId": "rank20-final-readonly-review-20260903",
...
The decisions array must include all 11 candidate claims and no extra claims.
